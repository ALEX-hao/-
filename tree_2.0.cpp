...
{														//这里不想用递归，递归会导致数组的标志无法置零，影响后续使用
	...
	while (p || stack_tree_bond[top_tree_stack])
	{
		...
		if (stack_tree_bond[top_tree_stack])
		{
			p = pop_stack();
...
{
	if (!p) { cout << "查找错误，未找到数据" << endl; return NULL; }
	if (a > p->data) return find_tree_bond(a, p->right);
	else if (a < p->data) return find_tree_bond(a, p->left);
	else return p;
}
template <typename T>
bond<T> *tree<T>::insert_tree_bond(T a, bond<T> *p)		//向树中插入元素并保证查找树的顺序，递归的返回指针
{
	...
	else if (a > p->data) p->right = insert_tree_bond(a, p->right);
	else if (a < p->data) p->left = insert_tree_bond(a, p->left);
	return p;
}
